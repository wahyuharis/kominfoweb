<script>
    ...
    $("input[name=image_upload]").change(function() {
        ...
        if (files.length > 0) {
            ...
            $.ajax({
                url: '<?= base_url('admin/video/upload/') ?>',
                type: 'post',
                data: fd,
                contentType: false,
                processData: false,
                success: function(response) {
                    if (response['success']) {
                        ...
                        $('#image_delete').click(function() {
                            $('#image_upload').show();
                            $.get("<?= base_url('admin/video/delete_image/') ?>" + output['file_name'], function(data, status) {
    ...
    $("input[name=image_upload2]").change(function() {
        ...
        if (files.length > 0) {
            ...
            $.ajax({
                url: '<?= base_url('admin/video/upload2/') ?>',
                type: 'post',
                data: fd,
                contentType: false,
                processData: false,
    ...
    function delete_image_bind() {
        $('.delete_image_list').click(function() {
            ...
            $.get("<?= base_url('admin/video/delete_image/') ?>" + image_name, function(data, status) {
    ...
    $('#form-1').submit(function(e) {
        ...
        $.ajax({
            url: '<?= base_url('admin/video/submit/') ?>',
            type: 'post',
