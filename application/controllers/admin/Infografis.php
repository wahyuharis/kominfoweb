...
class Infografis extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $crud->display_as('get_view(infografis.id)', 'View');
        ...
        if ($crud->getstate() == 'insert_validation') {
            $crud->set_rules('slug', 'Slug', 'trim|required|is_unique[infografis.slug]');
        }
        ...
        $content = $this->load->view('admin/infografis/infografis', $view_data, true);
    ...
    public function _callback_actions($value, $row)
    {
        ...
        $html .= "<a class='btn btn-xs btn-warning' href='" . base_url('admin/infografis/edit/' . $row->id) . "'>
        ...
        // if ($row->category == 'Infografis') {
        $html .= " <a class='btn btn-xs btn-default' href='" . base_url('admin/infografis/up/' . $row->id) . "'>
            <i class='fa fa-chevron-circle-up'></i>
            UP</a> ";

        $html .= "<a class='btn btn-xs btn-default' href='" . base_url('admin/infografis/down/' . $row->id) . "'>
    ...
    function edit($id = "")
    {
        ...
        $content_data['category'] = 'Infografis';
        $content_data['title'] = '';
        $content_data['slug'] = '';
        ...
        $content_data['category_opt'] = array('Infografis' => 'Infografis', 'Draft' => 'Draft');

        if (!empty(trim($id))) {
            $db =  $this->db
                ->where('infografis.id', $id)
                ->get('infografis');
        ...
        $content = $this->load->view('admin/infografis/infografis_edit', $content_data, true);
    ...
    function submit()
    {
        ...
        if ($succes) {
            ...
            if (empty(trim($primary_id))) {
                ...
                $this->db->insert('infografis', $insert);
    ...
    function down($id)
    {
        ...
        if ($db->num_rows() > 0 && $db2->num_rows() > 0) {
            ...
            if ($current['date'] != $down['date']) {
                ...
                $this->db->delete('infografis', array('id' => $id_down));
                $this->db->delete('infografis', array('id' => $id_current));

                $this->db->insert('infografis', $down);
                $this->db->insert('infografis', $current);
    ...
    function up($id)
    {
        ...
        if ($db->num_rows() > 0 && $db2->num_rows() > 0) {
            ...
            if ($current['date'] != $up['date']) {
                ...
                $this->db->delete('infografis', array('id' => $id_up));
                $this->db->delete('infografis', array('id' => $id_current));

                $this->db->insert('infografis', $up);
                $this->db->insert('infografis', $current);
